...
void CL_WriteDemoMessage (void)
{
	...
	d_write (cls.demofile, &swlen, 4);
	d_write (cls.demofile, net_message.data+8,	len);
}
...
// finish up
	len = -1;
	d_write (cls.demofile, &len, 4);
	d_close (cls.demofile);
	cls.demofile = -1;
	...
	for (i=0 ; i<MAX_CONFIGSTRINGS ; i++)
	{
		if (cl.configstrings[i][0])
		{
			...
			{	// write it out
				...
				d_write (cls.demofile, &len, 4);
				d_write (cls.demofile, buf.data, buf.cursize);
				buf.cursize = 0;
			}
...
void CL_WriteConfiguration (void)
{
	...
	d_printf (f, "// generated by quake, do not modify\n");
	Key_WriteBindings (f);
	d_close (f);
