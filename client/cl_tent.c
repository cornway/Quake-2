...
{
	d_memset (cl_beams, 0, sizeof(cl_beams));
	d_memset (cl_explosions, 0, sizeof(cl_explosions));
	d_memset (cl_lasers, 0, sizeof(cl_lasers));

//ROGUE
	d_memset (cl_playerbeams, 0, sizeof(cl_playerbeams));
	d_memset (cl_sustains, 0, sizeof(cl_sustains));
//ROGUE
}
...
explosion_t *CL_AllocExplosion (void)
{
	...
	for (i=0 ; i<MAX_EXPLOSIONS ; i++)
	{
		...
		{
			d_memset (&cl_explosions[i], 0, sizeof (cl_explosions[i]));
...
// find the oldest explosion
	...
	d_memset (&cl_explosions[index], 0, sizeof (cl_explosions[index]));
